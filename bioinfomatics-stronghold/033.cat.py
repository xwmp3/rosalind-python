    ...
    if seq not in cache:
        ...
        for k in range(1, len(seq), 2):
            ...
            * second half
            ...
            '''
            temp.append(noncrossing_perfect_matches_recursive(seq[1:k], cache) * cache[
                seq[0] + seq[k]] * noncrossing_perfect_matches_recursive(seq[k + 1:], cache))
        cache[seq] = sum(temp)
    return cache[seq]


if __name__ == '__main__':
    path = "datasets/033.cat.in"
    modulo = 10 ** 6
    seq = load_data(path)
    init_cache = {
        '': 1,
